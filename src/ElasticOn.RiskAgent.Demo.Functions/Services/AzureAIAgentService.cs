...
public class AzureAIAgentService : IAzureAIAgentService
{
    ...
    public AzureAIAgentService(IConfiguration configuration, ILogger<AzureAIAgentService> logger)
    {
        ...
        _agentName = configuration["AIServices:Agent:Name"] 
            ?? throw new InvalidOperationException("AIServices:Agent:Name is not configured");
        
        _agentInstructions = configuration["AIServices:Agent:Instructions"] 
            ?? throw new InvalidOperationException("AIServices:Agent:Instructions is not configured");

        // Read MCP tool configuration
        _mcpServerLabel = configuration["AIServices:MCPTool:ServerLabel"] 
            ?? throw new InvalidOperationException("AIServices:MCPTool:ServerLabel is not configured");
        
        _mcpServerUrl = configuration["AIServices:MCPTool:ServerUrl"] 
            ?? throw new InvalidOperationException("AIServices:MCPTool:ServerUrl is not configured");
        
        _mcpAllowedTools = configuration.GetSection("AIServices:MCPTool:AllowedTools").Get<List<string>>() 
            ?? throw new InvalidOperationException("AIServices:MCPTool:AllowedTools is not configured");

        // Read Elastic API key
        _elasticApiKey = configuration["AIServices:ElasticApiKey"] 
            ?? throw new InvalidOperationException("AIServices:ElasticApiKey is not configured");
    ...
    {
        // First priority: Check if there's a pre-configured Agent ID in configuration
        if (!string.IsNullOrEmpty(_configuredAgentId))
        {
            try
            {
                ...
                {
                    _logger.LogInformation("Using pre-configured agent from configuration with ID: {AgentId}", _configuredAgentId);
